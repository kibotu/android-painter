...
import net.kibotu.android.painter.R;
...
public class ColorPickerDialog extends Dialog {
    ...
    private static class ColorPickerView extends View {
        ...
        protected void onDraw ( Canvas canvas ) {
            int translatedHue = 255 - ( int ) ( mCurrentHue * 255 / 360 );
            // Display all the colors of the hue bar with linePaths
            for ( int x = 0; x < 256; x++ ) {
...
//			canvas.drawText(getResources().getString(R.string.settings_default_color_confirm), 202, 340, mPaint);
        ...
        public boolean onTouchEvent ( MotionEvent event ) {
            ...
            if ( x > 10 && x < 138 && y > 316 && y < 356 )
                mListener.colorChanged( "currentColor", mCurrentColor );

            // If the touch event is located in the right button, notify the listener with the default color
            if ( x > 138 && x < 266 && y > 316 && y < 356 )
                mListener.colorChanged( "defaultColor", mDefaultColor );
    ...
    protected void onCreate ( Bundle savedInstanceState ) {
        ...
        OnColorChangedListener l = new OnColorChangedListener() {
            public void colorChanged ( String key, int color ) {
                mListener.colorChanged( mKey + "_" + key, color );
                dismiss();
            }
        };
